...
expession_data <- read.csv(file = "./rawData/expression_CCLE.csv", sep = ",", row.names = NULL, header = T)
...
write.table(scores,"./NCScore/Hypoxia_scoresCCLE.csv",quote = FALSE,sep=",",row.names=T,col.names=TRUE)

library(ggplot2)
library(ggpubr)
scores <- data.frame(apply(scores, 2, as.numeric))
b <- ggplot(scores, aes(x = scores$Buffa, y = scores$Winter)) + 
...
target <- "Buffa&Winter"
ggsave(paste0("./NCScore/figure/",target,"_correlationCCLE.png"),b,width = 4,height = 4,device = "png",dpi = 900,units = "in")
